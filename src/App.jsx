...
const Navbar = ({ onOpenModal }) => {
  ...
  return (
    <nav className="fixed top-4 md:top-6 left-1/2 -translate-x-1/2 z-50 transition-all duration-500 rounded-[2rem] px-4 py-3 md:px-6 w-[95%] md:w-[90%] max-w-4xl" ref={navRef} style={{ border: '1px solid transparent' }}>
      <div className="flex items-center justify-between">
        <button
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="font-sans font-bold text-sm md:text-lg tracking-tighter text-white hover:text-accent transition-colors focus:outline-none"
        >
          STUDIO LEGALE VILLA
        </button>
        ...
        <button onClick={onOpenModal} className="magnetic-btn bg-accent text-background font-semibold px-4 py-2 md:px-5 md:py-2.5 rounded-full text-xs md:text-sm whitespace-nowrap ml-2">
          Prenota <span className="hidden sm:inline">Consulenza</span>
        </button>
      </div>
    </nav>
  );
...
const Hero = ({ onOpenModal }) => {
  ...
  return (
    <section className="relative min-h-[100dvh] flex items-end pb-24 px-8 md:px-16 overflow-hidden">
      ...
      {/* Content */}
      <div className="relative z-10 w-full max-w-5xl pt-20 md:pt-0" ref={textRef}>
        <div className="hero-text text-accent font-mono text-xs md:text-sm uppercase tracking-widest mb-4 md:mb-6 block">
          Due generazioni di assistenza legale
        </div>
        <h1 className="hero-text text-4xl sm:text-5xl md:text-7xl lg:text-[5.5rem] tracking-tighter leading-[1] md:leading-[0.9] text-white">
          <span className="font-sans font-bold">L'assistenza legale su misura porta</span><br />
          <span className="text-drama text-4xl sm:text-5xl md:text-[6rem] block mt-2 md:mt-3">alla soddisfazione delle tue esigenze.</span>
        </h1>
...
const CivileShuffler = () => {
  ...
  return (
    <div className="liquid-glass rounded-[2rem] p-6 md:p-8 h-[400px] flex flex-col relative overflow-hidden group">
      <h3 className="font-sans font-bold text-xl md:text-2xl text-white mb-2">Diritto Civile</h3>
      <p className="text-xs md:text-sm text-zinc-400 mb-8">Gestione rigorosa del contenzioso e consulenza stragiudiziale.</p>
...
const PenaleTypewriter = () => {
  ...
  return (
    <div className="liquid-glass rounded-[2rem] p-6 md:p-8 h-[400px] flex flex-col relative overflow-hidden">
      <div className="absolute top-4 right-4 md:top-6 md:right-6 flex items-center gap-2 border border-accent/20 px-2 md:px-3 py-1.5 rounded-full bg-accent/5">
        <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-accent animate-pulse"></div>
        <span className="font-mono text-[8px] md:text-[10px] tracking-widest uppercase text-accent">Live Feed</span>
      </div>
      <h3 className="font-sans font-bold text-xl md:text-2xl text-white mb-2">Diritto Penale</h3>
      <p className="text-xs md:text-sm text-zinc-400 mb-6">Massima precisione nell'impostazione difensiva e tutela dei diritti.</p>
...
const CostituzionaleScheduler = () => {
  ...
  return (
    <div className="liquid-glass rounded-[2rem] p-6 md:p-8 h-[400px] flex flex-col relative overflow-hidden group" ref={containerRef}>
      <h3 className="font-sans font-bold text-xl md:text-2xl text-white mb-2">Diritto Costituzionale</h3>
      <p className="text-xs md:text-sm text-zinc-400 mb-6 md:mb-8">Pianificazione accurata dei ricorsi e tutela multilivello.</p>

      <div className="relative border border-white/5 rounded-xl bg-surface/30 p-4 md:p-6 flex flex-col items-center">
        <div className="flex gap-1 md:gap-2 w-full justify-between mb-6 md:mb-8">
          {days.map((d, i) => (
            <div key={i} className={`day-cell-${i} w-6 h-6 md:w-8 md:h-8 rounded shrink-0 border border-white/5 flex items-center justify-center font-mono text-[10px] md:text-xs text-zinc-500`}>
              {d}
            </div>
          ))}
        </div>
...
const Features = () => {
  return (
    <section id="expertise" className="py-24 md:py-32 px-6 md:px-16 relative">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 md:mb-16">
          <h2 className="text-2xl md:text-5xl font-sans font-bold tracking-tight mb-3 md:mb-4">Competenze Strategiche</h2>
          <p className="text-sm md:text-base text-zinc-400 max-w-lg">Strumenti e domini legali di nostra pertinenza, affrontati con rigore metodologico.</p>
        </div>
...
const Philosophy = () => {
  ...
  return (
    <section id="philosophy" className="py-24 md:py-40 bg-zinc-950 relative overflow-hidden" ref={philRef}>
      {/* Texture bg */}
      <img
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-16 relative z-10 text-center flex flex-col items-center">
        <p className="phil-line text-base md:text-2xl text-zinc-500 font-sans mb-6 md:mb-8">
        ...
        <h2 className="phil-line text-3xl sm:text-4xl md:text-[5.5rem] leading-[1.1] text-ivory">
          Noi ci concentriamo su:<br />
          <span className="text-drama text-4xl sm:text-5xl md:text-[7rem] mt-2 md:mt-4 block">strategie legali proattive.</span>
        </h2>
      </div>
    </section>
  );
...
const Protocol = () => {
  ...
  return (
    <section id="protocol" className="py-16 md:py-24 relative" ref={containerRef}>
      <div className="max-w-4xl mx-auto px-6 md:px-8 relative h-full">
        {steps.map((step, i) => (
          <div key={i} className="protocol-card min-h-auto md:min-h-[50vh] liquid-glass rounded-[2rem] md:rounded-[3rem] p-8 md:p-12 mb-8 md:mb-16 flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-12 static">
            <div className="shrink-0 w-16 h-16 md:w-24 md:h-24 rounded-full border border-white/10 flex items-center justify-center bg-surface/30">
              {React.cloneElement(step.icon, { className: 'w-8 h-8 md:w-10 md:h-10 text-accent' })}
            </div>
            <div>
              <div className="font-mono text-accent text-xs md:text-sm mb-2 md:mb-4">FASE {step.num}</div>
              <h3 className="text-2xl md:text-4xl font-sans font-bold text-white mb-2 md:mb-4">{step.title}</h3>
              <p className="text-zinc-400 text-sm md:text-lg max-w-md">{step.desc}</p>
            </div>
          </div>
        ))}
      </div>
...
const FooterCTA = ({ onOpenModal }) => {
  return (
    <footer className="bg-[#0A0A0F] pt-20 md:pt-32 pb-8 md:pb-12 px-6 md:px-16 mt-16 md:mt-24 rounded-t-[2rem] md:rounded-t-[4rem] relative overflow-hidden">
      <div className="max-w-7xl mx-auto relative z-10">

        {/* CTA Banner */}
        <div className="liquid-glass rounded-[2rem] p-8 md:p-12 mb-16 md:mb-24 flex flex-col items-center text-center">
          <h2 className="text-2xl md:text-5xl font-bold text-white mb-4 md:mb-6">Pronto per iniziare?</h2>
          <p className="text-zinc-400 text-sm md:text-base max-w-lg mb-6 md:mb-8">Prenota una consultazione iniziale. Esamineremo i dettagli del tuo caso e stabiliremo il protocollo di azione ottimale.</p>
          <button onClick={onOpenModal} className="magnetic-btn bg-accent text-background font-semibold px-6 py-3 md:px-10 md:py-4 rounded-full text-sm md:text-lg">
            Consultazione Strategica
          </button>
        </div>

        {/* Footer Grid */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-12 border-t border-white/5 pt-10 md:pt-12">
...
const BookingModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Finestra Modal */}
      <div className="liquid-glass border border-white/10 rounded-[1.5rem] md:rounded-[2rem] w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 md:p-8 relative z-10 shadow-2xl animate-in fade-in zoom-in duration-300">
        ...
        {status === 'success' ? (
          ...
          <>
            <h3 className="text-xl md:text-2xl font-sans font-bold mb-2 flex items-center gap-2 md:gap-3 text-white pr-8">
              <span className="w-2.5 h-2.5 rounded-full bg-accent animate-pulse block shrink-0"></span>
              Pianificazione Strategica
            </h3>
            <p className="text-zinc-400 text-xs md:text-sm mb-6 md:mb-8 relative pr-4 md:pr-8">
